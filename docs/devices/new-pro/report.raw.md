...
|------|------|-------|
| `/` | 4825 B |  |
| `/bitcoin.html` | 10355 B |  |
| `/coingecko.html` | 10364 B |  |
| `/filelist?dir=` | 12 B |  |
| `/image.html` | 12682 B |  |
| `/monitor.html` | 1729 B |  |
| `/network.html` | 4825 B |  |
| `/settings.html` | 7033 B |  |
| `/stock.html` | 6370 B |  |
| `/time.html` | 4729 B |  |
| `/weather.html` | 12351 B |  |
| `/wifi.json` | 538 B |  |
...
- Sample response:
  ...
  ```
- Evidence: `html_onload` in `/settings.html`; `runtime_request` in `runtime:/settings.html`

### `GET /.sys/city.json`
- Content-Type: `text/plain` — 81 B
- Schema:
...
- Sample response:
  ...
  ```
- Evidence: `html_onload` in `/`; `js_getData` in `/`; `html_onload` in `/network.html`; `js_getData` in `/network.html`; `runtime_request` in `runtime:/`; `runtime_request` in `runtime:/`; (+2 more)

### `GET /.sys/monitor.json`
- Content-Type: `text/plain` — 40 B
- Schema:
...
- Sample response:
  ...
  ```
- Evidence: `html_onload` in `/monitor.html`; `runtime_request` in `runtime:/monitor.html`

### `GET /.sys/stock2.json`
- Content-Type: `text/plain` — 250 B
- Schema:
...
- Sample response:
  ...
  ```
- Evidence: `runtime_request` in `runtime:/stock.html`
...
- Sample response:
  ...
  ```
- Evidence: `js_getData` in `/`; `js_rel_file` in `/`; `js_getData` in `/network.html`; `js_rel_file` in `/network.html`; `runtime_request` in `runtime:/`; `runtime_request` in `runtime:/`; (+2 more)

## Other read endpoints (text/binary, safely probed)

### `GET /css/cropper.min.css`
- Content-Type: `text/css` — 3742 B
- Preview:
...
- Preview:
  ...
  ```
- Evidence: `html_href` in `/`; `html_href` in `/network.html`; `html_href` in `/weather.html`; `html_href` in `/time.html`; `html_href` in `/image.html`; `html_href` in `/stock.html`; (+14 more)
...
- Content-Type: `text/javascript` — 89493 B
- Preview:
  ...
  ... (truncated 56725 bytes)
  ```
- Evidence: `html_src` in `/`; `html_src` in `/network.html`; `html_src` in `/weather.html`; `html_src` in `/image.html`; `html_src` in `/bitcoin.html`; `html_src` in `/coingecko.html`; (+6 more)

### `GET /js/settings.js`
- Content-Type: `text/javascript` — 9526 B
- Preview:
  ...
  var cin8 = getE("cin8");
  var cin9 = getE("cin9");
  var cin10 = getE("cin10");
  var cin11 = getE("cin11");
  var kline = getE("kline");
  var coin_ani = getE("coin_ani");
  var coin_interval = getE("coin_interval");
  var c_l = getE("c_loop");
  var rot_i = getE("rot_i");
  ...
  var gin8 = getE("gin8");
  var gin9 = getE("gin9");
  var gin10 = getE("gin10");
  var gin11 = getE("gin11");
  var g_range = getE("g_range");
  var g_ani = getE("g_ani");
  var g_i = getE("g_i");
  var g_l = getE("g_l");
  var g_loading = getE("g_loading");
  var g_clock = getE("g_clock");
  var g_rot_i = getE("g_rot_i");
  
  //监视器
  var ip = getE("ip");
  var m_i = getE("m_i");
  document.title = "GEEKMAGIC PRO";
  function getData(data) {
  	...
  	//todo 单位等。
  	if (res.nickname) {
  		const nickname = getE("nickname");
  		if (nickname) nickname.value = res.nickname;
  	}
  	
  	//设置
  	if(res.brt) brt.value = 255-res.brt;
  	...
  	if(res.b2) time_brt.value = 255-res.b2;
  	...
  	if(res.s_ani == "1") stock_ani.checked = true;
  	
  	//coingecko
  	if(res.gin0) gin0.value = res.gin0;
  	if(res.gin1) gin1.value = res.gin1;
  	if(res.gin2) gin2.value = res.gin2;
  	if(res.gin3) gin3.value = res.gin3;
  	if(res.gin4) gin4.value = res.gin4;
  	if(res.gin5) gin5.value = res.gin5;
  	if(res.gin6) gin6.value = res.gin6;
  	if(res.gin7) gin7.value = res.gin7;
  	if(res.gin8) gin8.value = res.gin8;
  	if(res.gin9) gin9.value = res.gin9;
  	if(res.gin10) gin10.value = res.gin10;
  	if(res.gin11) gin11.value = res.gin11;
  	if(res.g_l) g_l.checked = true;
  	if(res.g_i) g_i.value = res.g_i;
  	if(res.range) g_range.value= res.range;
  	if(res.g_rot_i) g_rot_i.value = res.g_rot_i;
  
  	//加密货币
  	if(res.cin0) cin0.value = res.cin0;
  	if(res.cin1) cin1.value = res.cin1;
  	if(res.cin2) cin2.value = res.cin2;
  	if(res.cin3) cin3.value = res.cin3;
  	if(res.cin4) cin4.value = res.cin4;
  	if(res.cin5) cin5.value = res.cin5;
  	if(res.cin6) cin6.value = res.cin6;
  	if(res.cin7) cin7.value = res.cin7;
  	if(res.cin8) cin8.value = res.cin8;
  	if(res.cin9) cin9.value = res.cin9;
  	if(res.cin10) cin10.value = res.cin10;
  	if(res.cin11) cin11.value = res.cin11;
  	if(res.k) kline.value = res.k;
  	if(res.rot_i) rot_i.value = res.rot_i;
      if (res.thousand_sep || res.decimal_sep) {
          if (res.thousand_sep) getE("thousand_separator").value = res.thousand_sep;
          if (res.decimal_sep) getE("decimal_separator").value = res.decimal_sep;
      }
  
  	if(res.c_loading){
  		const cloading = getE("coin_loading");
  		if (res.c_loading) cloading.checked = true;
  	}
  	if(res.c_clock){
  		const clock= getE("coin_clock");
  		if (res.c_clock) clock.checked = true;
  	}
  
  	if (res.up_fg) {
  		const upFgColor = getE("up_fg_color");
  		if (upFgColor) upFgColor.value = res.up_fg;
  	}
  	if (res.up_bg) {
  		const upBgColor = getE("up_bg_color");
  		if (upBgColor) upBgColor.value = res.up_bg;
  	}
  	if (res.down_fg) {
  		const downFgColor = getE("down_fg_color");
  		if (downFgColor) downFgColor.value = res.down_fg;
  	}
  	if (res.down_bg) {
  		const downBgColor = getE("down_bg_color");
  		if (downBgColor) downBgColor.value = res.down_bg;
  	}
  	
  	if(res.c_i) coin_interval.value = res.c_i;
  	if(res.c_l) c_l.checked = true;
  	if(res.c_ani) coin_ani.checked = true;
  ...
  function send_http(url){
  	getResponse(url, function(responseText) {
  		if (responseText == "OK") {
  			showPopup("Success!", 1500, "#02a601");
  		} else{
  			if (url.includes('/wifisave')) {
  				showPopup("WiFi info saved, will reboot to connect!", 3000, "#02a601");
  			}else{
  				showPopup("Failed.", 2000, "#dc0d04");
  			}
  		} 
  	});
  }
  function set_c(){
  	...
  	if(copyright != null)copyright.innerHTML = '<br />copyright (c) 2025 GeekMagic®<br /><a href=\"\" target=\"_blank\">Support mail: GeekMagic@163.com</a>';
  }
  set_c();
  ...
  function getNav(){
  	var navLinks = [
  		...
  		{ href: "image.html", text: "Picture" },
  		{ href: "stock.html", text: "Stock" },
  		{ href: "bitcoin.html", text: "Bitcoin" },
  		{ href: "coingecko.html", text: "CoinGecko" },
  		{ href: "monitor.html", text: "Monitor" },
  		{ href: "settings.html", text: "System" }
  	];
  ...
  function getResponse(adr, callback, timeoutCallback, timeout, method){
  	if(timeoutCallback === undefined) {
  		timeoutCallback = function(){
  			showPopup("Send request time out, please refresh... ", 2000, "#dc0d04");
  		};
  	}
  ...
  ```
- Evidence: `html_src` in `/`; `html_src` in `/network.html`; `html_src` in `/weather.html`; `html_src` in `/time.html`; `html_src` in `/image.html`; `html_src` in `/image.html`; (+16 more)
...
### `GET /set`
- Query parameters observed: `brt`, `c0`, `c_chart`, `c_theme`, `cd1`, `clear`, `day`, `delay`, `enter`, `g_range`, `g_theme`, `gif`, `hc`, `hour`, `i_i`, `img`, `ip`, `key`, `kline`, `ntp`, `page`, `reboot`, `reset`, `st_kline`, `t1`, `theme`, `time_interval`, `w_u`
- Example URLs observed:
  - `/set?delay=`
  - `/set?key=`
  - `/set?w_u=`
  ...
  - `/set?day=`
  - `/set?hc=`
- Evidence: `js_literal` in `/`; `js_literal` in `/network.html`; `js_literal` in `/weather.html`; `js_literal` in `/weather.html`; `js_literal` in `/weather.html`; `js_literal` in `/weather.html`; (+25 more)

### `GET /update`
- Evidence: `html_href` in `/settings.html`
...
- Example URLs observed:
  - `/wifisave?s=`
- Evidence: `js_literal` in `/`; `js_literal` in `/js/settings.js`; `js_literal` in `/network.html`

## Probed but not served (likely dead code or removed feature)

- `/.sys/album.json` → HTTP 404 (`text/plain`) — evidence: `html_onload` in `/image.html`; `runtime_request` in `runtime:/image.html`
- `/.sys/app.json` → HTTP 404 (`text/plain`) — evidence: `html_onload` in `/settings.html`; `runtime_request` in `runtime:/settings.html`
- `/.sys/ccolor.json` → HTTP 404 (`text/plain`) — evidence: `html_onload` in `/bitcoin.html`; `html_onload` in `/coingecko.html`; `runtime_request` in `runtime:/bitcoin.html`; `runtime_request` in `runtime:/coingecko.html`
- `/.sys/cg_range.json` → HTTP 404 (`text/plain`) — evidence: `html_onload` in `/coingecko.html`; `runtime_request` in `runtime:/coingecko.html`
- `/.sys/cgk.json` → HTTP 404 (`text/plain`) — evidence: `html_onload` in `/coingecko.html`; `runtime_request` in `runtime:/coingecko.html`
- `/.sys/coin2.json` → HTTP 404 (`text/plain`) — evidence: `html_onload` in `/bitcoin.html`; `runtime_request` in `runtime:/bitcoin.html`
- `/.sys/cr.json` → HTTP 404 (`text/plain`) — evidence: `html_onload` in `/bitcoin.html`; `runtime_request` in `runtime:/bitcoin.html`
- `/.sys/day.json` → HTTP 404 (`text/plain`) — evidence: `html_onload` in `/time.html`; `runtime_request` in `runtime:/time.html`
- `/.sys/delay.json` → HTTP 404 (`text/plain`) — evidence: `js_getData` in `/`; `js_getData` in `/network.html`; `html_onload` in `/settings.html`; `runtime_request` in `runtime:/`; `runtime_request` in `runtime:/network.html`; `runtime_request` in `runtime:/settings.html`
- `/.sys/hour12.json` → HTTP 404 (`text/plain`) — evidence: `html_onload` in `/time.html`; `runtime_request` in `runtime:/time.html`
- `/.sys/k.json` → HTTP 404 (`text/plain`) — evidence: `html_onload` in `/bitcoin.html`; `runtime_request` in `runtime:/bitcoin.html`
- `/.sys/key.json` → HTTP 404 (`text/plain`) — evidence: `html_onload` in `/weather.html`; `runtime_request` in `runtime:/weather.html`
- `/.sys/ntp.json` → HTTP 404 (`text/plain`) — evidence: `html_onload` in `/time.html`; `runtime_request` in `runtime:/time.html`
- `/.sys/sep.json` → HTTP 404 (`text/plain`) — evidence: `html_onload` in `/bitcoin.html`; `html_onload` in `/coingecko.html`; `runtime_request` in `runtime:/bitcoin.html`; `runtime_request` in `runtime:/coingecko.html`
- `/.sys/stk.json` → HTTP 404 (`text/plain`) — evidence: `runtime_request` in `runtime:/stock.html`
- `/.sys/timebrt.json` → HTTP 404 (`text/plain`) — evidence: `html_onload` in `/monitor.html`; `html_onload` in `/settings.html`; `runtime_request` in `runtime:/monitor.html`; `runtime_request` in `runtime:/settings.html`
- `/.sys/timecolor.json` → HTTP 404 (`text/plain`) — evidence: `html_onload` in `/time.html`; `runtime_request` in `runtime:/time.html`
- `/.sys/unit.json` → HTTP 404 (`text/plain`) — evidence: `html_onload` in `/weather.html`; `runtime_request` in `runtime:/weather.html`

---
_Discovered 48 endpoints across 43 fetched resources._
